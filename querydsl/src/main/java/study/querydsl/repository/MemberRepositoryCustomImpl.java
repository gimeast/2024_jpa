...
public class MemberRepositoryCustomImpl extends QuerydslRepositorySupport implements MemberRepositoryCustom {
    ...
    public Page<MemberTeamDto> searchPageSimple2(MemberSearchCondition condition, Pageable pageable) {
        ...
        QueryResults<MemberTeamDto> results1 = query.fetchResults();
        List<MemberTeamDto> content = results1.getResults();
        long total = results1.getTotal();
...
//        return new PageImpl<>(content, pageable, getTotal(condition));
    ...
    private int getTotal(MemberSearchCondition condition) {
        return queryFactory
                .select(member)
                .from(member)
//                .leftJoin(member.team, team) //content와 count를 분리 시키면 필요없는 join을 제외하고 할 수 있다.
                .where(
                        usernameEq(condition.getUsername()),
                        teamNameEq(condition.getTeamName()),
                        ageGoe(condition.getAgeGoe()),
